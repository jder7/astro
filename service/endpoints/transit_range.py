...
from service.schemas import TransitRangeRequest, TransitRangeResponse, TransitRangeSnapshot, BirthData
from service.utils import ensure_config, build_subject_for_moment, compute_major_aspects, compute_normal_aspects, to_local_datetime, iter_range_datetimes
...
async def transit_range(payload: TransitRangeRequest) -> TransitRangeResponse:
    ...
    """
    print("POST /transit-range", payload.model_dump(exclude_none=True))
    ...
    for dt in iter_range_datetimes(start_dt, end_dt, payload.granularity):
        ...
        moment_aspects = (
            compute_normal_aspects(moment_subject, active_points=cfg.active_points)
            if payload.include_aspects
            else []
        )
