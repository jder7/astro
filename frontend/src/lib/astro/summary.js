...
function collectRanges(response) {
  ...
  const ranges = [];
  const pointRanges = response.point_sign_range || response.pointSignRange || [];
  if (Array.isArray(pointRanges) && pointRanges.length) {
    pointRanges.forEach((range) => {
      const entries = Array.isArray(range.entries) ? range.entries : [];
      if (!entries.length) return;
      const pointKey = range.point_key || range.pointKey || 'point';
      const labelParts = [range.label, range.point_label || range.pointLabel].filter(Boolean);
      ranges.push({
        key: pointKey,
        label: labelParts.length ? labelParts.join(' · ') : String(pointKey || 'range'),
...
function simplifyAspect(entry) {
  ...
  const leftBase = fallback(entry.left, entry.first_point, entry.inner_point, entry.point_a, entry.planet_a, entry.p1_name);
  const rightBase = fallback(entry.right, entry.second_point, entry.outer_point, entry.point_b, entry.planet_b, entry.p2_name);
  const leftOwner = entry.p1_owner || '';
  const rightOwner = entry.p2_owner || '';
  const left = leftOwner && leftBase && !String(leftBase).includes(leftOwner)
    ? `${leftBase} (${leftOwner})`
    : leftBase;
  const right = rightOwner && rightBase && !String(rightBase).includes(rightOwner)
    ? `${rightBase} (${rightOwner})`
    : rightBase;
...
const normalizeRelationshipAspects = (resp) => {
  ...
  if (resp.synastry && Array.isArray(resp.synastry.dual_chart_aspects)) return resp.synastry.dual_chart_aspects;
  if (resp.synastry && Array.isArray(resp.synastry.aspects_list)) return resp.synastry.aspects_list;
  if (resp.synastry && Array.isArray(resp.synastry.aspect_list)) return resp.synastry.aspect_list;
  if (resp.aspects && typeof resp.aspects === 'object' && !Array.isArray(resp.aspects)) {
    ...
    if (Array.isArray(resp.aspects.dual_chart_aspects)) return resp.aspects.dual_chart_aspects;
    if (Array.isArray(resp.aspects.aspects_list)) return resp.aspects.aspects_list;
    if (Array.isArray(resp.aspects.aspect_list)) return resp.aspects.aspect_list;
...
export function buildSummary(mode, response, birthParts, transitParts) {
  ...
  if (mode === 'natal' && response?.subject) {
    ...
    sections.push({
      meta: buildMeta(response.subject, 'Natal', 'birth'),
      points: extractPoints(response.subject),
      aspects: rawAspects,
    });
  } else if ((mode === 'transit' || mode === 'natal_transit') && response?.snapshot) {
    const snap = response.snapshot;
    const transitAspects = normalizeAspectsArray(snap.aspects);
    if (snap.subject) {
      sections.push({
        meta: buildMeta(snap.subject, 'Transit sky', 'transit'),
        points: extractPoints(snap.subject),
        aspects: transitAspects,
      });
    }
    if (snap.natal_subject) {
      sections.push({
        meta: buildMeta(snap.natal_subject, 'Natal', 'birth'),
        points: extractPoints(snap.natal_subject),
        aspects: normalizeAspectsArray(snap.natal_aspects),
      });
    }
    rawAspects = transitAspects;
    aspects = rawAspects.map(simplifyAspect).filter(Boolean);
