<script>
  ...
  export let showSynastryOnly = false;
  export let hideSynastryMatrix = false;
  ...
  const collectAspectKeys = (...groups) => {
    const set = new Set();
  ...
  const formatAspect = (entry, limit) => {
    ...
    if (Number.isFinite(orbVal) && !withinOrb(orbVal, limit)) return null;
    return {
      left,
      right,
      leftLabel,
  ...
  $: formatted = rawAspects.map((entry) => formatAspect(entry, orbLimit)).filter(Boolean);
  $: formattedSecondary = rawSecondaryAspects.map((entry) => formatAspect(entry, orbLimit)).filter(Boolean);
  $: formattedSyn = (synastry.aspects || []).map((entry) => formatAspect(entry, orbLimit)).filter(Boolean);
  $: labelMap = (() => {
    const map = new Map();
  ...
  $: matrixKeys = collectAspectKeys(formatted, formattedSecondary);
  $: synKeys = collectAspectKeys(formattedSyn);
  $: matrixRows = (() => {
    if (!matrixKeys.length || (!formatted.length && !formattedSecondary.length)) return [];
    ...
    return matrixKeys.map((rowKey, rowIdx) => {
      ...
      const cells = matrixKeys.map((colKey, colIdx) => {
        if (colIdx === rowIdx) {
...
{#if formattedSyn.length && synKeys.length && (!hideSynastryMatrix || showSynastryOnly)}
  <div class="adv-matrix-wrap mt-4" id="adv-aspects-synastry-matrix">
    <table class="adv-aspect-matrix w-full">
      <tbody>
        {#each synKeys as rowKey, rowIdx}
          <tr>
              {#each synKeys as colKey, colIdx}
                {#if colIdx === rowIdx}
...
{:else if showSynastryOnly}
  <p class="text-sm text-slate-400">No synastry aspects returned.</p>
{/if}
