<script>
  ...
  const summaryFor = (rangeIdx) => {
    ...
    const illumination = illuminationDisplay(seg.entry);
    const phaseIcon = phaseIconFor(seg.entry);
    const phaseLabel = phaseLabelFor(seg.entry);
    return {
      sign: seg.sign,
      element: seg.element,
      quality: seg.quality,
          ...
          {#if normalized.length === 1 && summaries[0]}
            <g transform="translate(210 210)" id="asc-summary-single">
              ...
              <g font-size="10" fill={ELEMENT_HEX[summaries[0].element] || ELEMENT_HEX.Default}>
                ...
                {#if summaries[0].illumination || summaries[0].phaseLabel}
                  <text id="asc-summary-phase" x="4" y="26" text-anchor="middle" font-size="9" fill="#cbd5e1">
                    {summaries[0].phaseIcon} {summaries[0].phaseLabel} {summaries[0].illumination}
                  </text>
                {/if}
              </g>
            </g>
              ...
              {#if summaries[i]}
                <g transform={`translate(${i === 0 ? 80 : 340} 390)`} id={`asc-summary-${i}`}>
                  ...
                  <g font-size="10" fill={HAND_COLORS[i] || HAND_COLORS[0]}>
                    ...
                    {#if summaries[i].illumination || summaries[i].phaseLabel}
                      <text id={`asc-summary-phase-${i}`} x="4" y="26" text-anchor="middle" font-size="9" fill="#cbd5e1">
                        {summaries[i].phaseIcon} {summaries[i].phaseLabel} {summaries[i].illumination}
                      </text>
                    {/if}
                  </g>
                </g>
